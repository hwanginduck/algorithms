...
//    그때의 폰켓몬 종류 번호의 개수를 return 하도록 solution 함수를 완성해주세요.
    ...
    public static int solution(int[] nums){
        ...
        // 최대한 가질 수 있는 포켓몬의 수는 절반이다.
        int half = nums.length/2;

        // 겹치는 포켓몬 번호에 대해서 정리를 해줄 HashSet 객체 생성
        HashSet<Integer> hashSet = new HashSet<>();

        // HashSet 안에 추가를 한다. (추가하는 과정 중에 겹치는 포켓몬 번호는 삭제)
        for(int num : nums){
            hashSet.add(num);
        }

        // 포켓몬의 절반 또는 겹치는 것을 제외한 숫자 중 더 작은 숫자에 대해서 반환을 해준다
